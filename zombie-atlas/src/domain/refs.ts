...
export interface RefSummary {
  ...
  classRows: number[][];
  rankings: {
    topCalled?: [number, number, number][];
    topCallers?: [number, number, number][];
    topWritten?: [number, number, number][];
    topRead?: [number, number, number][];
  };
}
      ...
      .then((raw) => {
        ...
        for (const [id, payload] of Object.entries(raw)) {
          ...
          for (const m of payload.members ?? []) {
            const row = m as [number, MemberRefs['kind'], string, RefRow[] | null, RefRow[] | null];
            byLine.set(row[0], { line: row[0], kind: row[1], name: row[2], out: row[3], in: row[4] });
          }
