...
function GH.execute(player, args)
    ...
    if not GH.heldHook(player) then return false, "UI_GH_Invalid" end
    ...
    if not window:addSheetRope(player, GH.ropeItem) then
        return false, "UI_GH_AttachFailed"
    end

    GH.log("rope tied", square:getX(), square:getY(), square:getZ(), "ropes spent", verdict.cost)
